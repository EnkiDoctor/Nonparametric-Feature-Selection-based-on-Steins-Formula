...
def DFS_epoch(model, s, supp_x, data, label, loss_func, optimizer0, optimizer, Ts, step=1):
    ...
    for j in range(Ts):
        with torch.no_grad():
            tmp = model.hidden0.weight.data.detach().cpu().numpy().copy() # Save the model parameters
        # updating W
        for _ in range(step):
            out = model(data)
            loss = loss_func(out, label)
            ...
            if len(Tc) > 0:
                with torch.no_grad():
                    model.hidden0.weight.data[Tc] = torch.from_numpy(tmp[Tc]).to(model.hidden0.weight.device)
        # updating w
        for _ in range(step):
            out = model(data)
            loss = loss_func(out, label)
...
def training_n(X, Y, X_test, Y_test, c, s, 
    ...
    for i in range(epochs):
        ...
        hist.append(lf(model(X), Y).data.cpu().numpy().tolist())
        SUPP.append(supp_x)
    ...
    bic = (loss.data.cpu().numpy().tolist())*N*2. + s*np.log(N) # bic based on final model
...
def training_l(X, Y, X_test, Y_test, supp, c, s,
    ...
    bic = N*np.log(loss.data.cpu().numpy().tolist()) + C*s*np.log(N) # bic based on final model
